...
STREAM_URL  = "https://rbse.rankguruji.com/api/stream"
...
def fetch_result():
    ...
    std   = data.get('std',   '10')
    payload = {"board": board, "year": year, "std": std, "roll_no": int(roll_no)}
...
def fetch_stream():
    ...
    std   = data.get('std',   '10')
    payload = {"board": board, "year": year, "std": std, "roll_no": int(roll_no)}
    print(f"[Stream] std={std} year={year} roll={roll_no}")
    def generate():
        try:
            with requests.post(STREAM_URL, json=payload, headers=FAKE_HEADERS, stream=True, timeout=120) as upstream:
...
def health():
    return jsonify({"status": "ok", "message": "Shiksha Vibhag API running"})
